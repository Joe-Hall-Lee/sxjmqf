...
A = [-2, 3];
b = 6;
[x, fval] = fmincon(@fun1, x0, A, b, [], [], [], [], @nonlfun1) % 注意 fun1.m 文件和 nonlfun1.m 文件都必须在当前文件夹目录下
fval = -fval
...
% MATLAB 2017a 默认使用的算法是'interior-point' 内点法
...
x0 = [1, 1]; % 任意给定一个初始值
[x, fval] = fmincon(@fun1, x0, A, b, [], [], [], [], @nonlfun1, option)
fval = -fval % 最大值为 -1，和内点法相同（这说明内点法的适应性要好）
...
b = 6;
for i = 1:n
    x0 = [rand() * 10, rand() * 10]; %用 随机数生成一个初始值（随机数的范围自己根据题目条件设置）
    [x, fval] = fmincon(@fun1, x0, A, b, [], [], [], [], @nonlfun1, option); % 注意 fun1.m 文件和 nonlfun1.m 文件都必须在当前文件夹目录下
...
% x =
%
%          0.552167405725893          1.20325915508114         0.947824046150062
%
%
% fval =
%
...
%% 例题三的求解（蒙特卡罗模拟那一讲的例题）
clear; clc
% 蒙特卡罗模拟得到的最大值为 3445.6012
% 最大值处 x1、x2、x3 的取值为：
%           22.5757290277919          12.5757290277919          12.1364047783332
A = [1, -2, -2; 1, 2, 2];
b = [0, 72];
x0 = [22.58, 12.58, 12.14];
Aeq = [1, -1, 0];
beq = 10;
lb = [-inf, 10, -inf];
ub = [inf, 20, inf];
