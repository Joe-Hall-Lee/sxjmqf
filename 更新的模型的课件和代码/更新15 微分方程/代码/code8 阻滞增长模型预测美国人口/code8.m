...
% （5）右边的"结果"一栏最上面显示"拟合计算未收敛"，左边的拟合图形也表明拟合结果不理想
% （6）点击"高级选项"，修改非线性最小二乘估计法拟合的起点（StartPoint），r 修改为 0.02，xm 修改为 500
% （7）此时右边的"结果"一栏得到了拟合结果：r = 0.0274, xm = 342.4450
% （8）依次点击拟合工具箱的菜单栏最右边的"导出"——"生成代码"（导出代码到时候可以放在论文附录），可以得到一个未命名的脚本文件
