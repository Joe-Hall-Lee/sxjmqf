%% 线性递减惯性权重的粒子群算法 PSO: 求解函数 y = x1^2 + x2^2 - x1 * x2 - 10 * x1 - 4 * x2 + 60 在 [-15, 15] 内的最小值（动画演示）
clear; clc
...
w_start = 0.9; % 初始惯性权重，通常取 0.9
w_end = 0.4; % 粒子群最大迭代次数时的惯性权重，通常取 0.4
K = 100; % 迭代的次数
vmax = [6, 6]; % 粒子的最大速度
x_lb = [-15, -15]; % x 的下界
x_ub = [15, 15]; % x 的上界
...
for d = 1:K % 开始迭代，一共迭代 K 次
    for i = 1:n % 依次更新第 i 个粒子的速度与位置
        ...
        if fit(i) < Obj_fun2(gbest) % 如果第 i 个粒子的适应度小于所有的粒子迄今为止找到的最佳位置对应的适应度
