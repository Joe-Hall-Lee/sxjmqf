%% 自适应权重且带有收缩因子的粒子群算法 PSO：求解四种不同测试函数的最小值（动画演示）
clear; clc
...
%% 改进：自动退出迭代循环
Count = 0; % 计数器初始化为 0
max_Count = 30; % 最大计数值初始化为 30
tolerance = 1e-6; % 函数变化量容忍度，取 10^(-6)
...
for i = 1:n % 循环整个粒子群，计算每一个粒子的适应度
    fit(i) = Obj_fun3(x(i, :)); % 调用 Obj_fun3 函数来计算适应度
end
...
for d = 1:K % 开始迭代，一共迭代 K 次
    ...
    for i = 1:n % 依次更新第 i 个粒子的速度与位置
        ...
        fit(i) = Obj_fun3(x(i, :)); % 重新计算第 i 个粒子的适应度
        if fit(i) < Obj_fun3(pbest(i, :)) % 如果第 i 个粒子的适应度小于这个粒子迄今为止找到的最佳位置对应的适应度
            pbest(i, :) = x(i, :); % 那就更新第 i 个粒子迄今为止找到的最佳位置
        end
        if fit(i) < Obj_fun3(gbest) % 如果第 i 个粒子的适应度小于所有的粒子迄今为止找到的最佳位置对应的适应度
            gbest = pbest(i, :); % 那就更新所有粒子迄今为止找到的最佳位置
        end
    end
    fitnessbest(d) = Obj_fun3(gbest); % 更新第 d 次迭代得到的最佳的适应度
    delta_fit = abs(Obj_fun3(gbest)-Obj_fun3(tem)); % 计算相邻两次迭代适应度的变化量
    if delta_fit < tolerance % 判断这个变化量和"函数变化量容忍度"的相对大小，如果前者小，则计数器加 1
        Count = Count + 1;
    else
        Count = 0; % 否则计数器清 0
    end
