%% 综合应用：粒子群算法 PSO: 求解函数 y = 21.5 + x(1) * sin(4*pi*x(1)) + x(2) * sin(20*pi*x(2) 在 x1 属于 [-3, 12.1]，x2 属于 [4.1, 5.8] 内的最大值（动画演示）
...
for i = 1:n % 循环整个粒子群，计算每一个粒子的适应度
    fit(i) = Obj_fun_2(x(i, :)); % 调用 Obj_fun_2 函数来计算适应度
end
pbest = x; % 初始化这n个粒子迄今为止找到的最佳位置（是一个 n * narvs 的向量）
...
for d = 1:K % 开始迭代，一共迭代 K 次
    for i = 1:n % 依次更新第 i 个粒子的速度与位置
        ...
        if f_i >= f_avg
            if f_avg ~= f_max % 如果分母为 0，我们就干脆令 w = w_min
        ...
        fit(i) = Obj_fun_2(x(i, :)); % 重新计算第 i 个粒子的适应度
        if fit(i) > Obj_fun_2(pbest(i, :)) % 如果第 i 个粒子的适应度大于这个粒子迄今为止找到的最佳位置对应的适应度
            pbest(i, :) = x(i, :); % 那就更新第 i 个粒子迄今为止找到的最佳位置
        end
        if fit(i) > Obj_fun_2(gbest) % 如果第 i 个粒子的适应度大于所有的粒子迄今为止找到的最佳位置对应的适应度
            gbest = pbest(i, :); % 那就更新所有粒子迄今为止找到的最佳位置
        end
    end
    fitnessbest(d) = Obj_fun_2(gbest); % 更新第 d 次迭代得到的最佳的适应度
    % pause(0.1) % 暂停 0.1 s
